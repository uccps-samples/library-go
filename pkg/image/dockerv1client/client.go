...
// API connections.
// TODO: accept a godockerclient auth config
...
// segment and godockerclient API version.
...
// by the godockerclient registry
...
func (repo *v2repository) getTags(c *connection) (map[string]string, error) {
	...
	case code == http.StatusUnauthorized:
		if len(repo.token) != 0 {
			...
			// godockerclient will not return a NotFound on any repository URL - for backwards compatibility, return NotFound on the
			// repo
...
func (repo *v2repository) unmarshalImageManifest(c *connection, body []byte) (*godockerclient.Image, error) {
...
// for v1 godockerclient registries.
...
func unmarshalDockerImage(body []byte) (*godockerclient.Image, error) {
	var imagePre012 godockerclient.ImagePre012
	if err := json.Unmarshal(body, &imagePre012); err != nil {
		return nil, err
	}

	return &godockerclient.Image{
		ID:              imagePre012.ID,
